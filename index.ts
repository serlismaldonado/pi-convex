...
interface ConvexConnection {
  ...
  authType: "deploy_key" | "anonymous" | null;
}

interface ConvexConfig {
  connections: Record<string, ConvexConnection>;
  active: string | null;
}
...
let config: ConvexConfig = { connections: {}, active: null };
...
function getActiveConnection(): ConvexConnection | null {
...
function buildDeployCommand(projectName: string, teamName?: string): { cmd: string; env: Record<string, string> } {
  if (!isInteractive()) {
...
function execCommand(cmd: string, cwd: string, signal?: AbortSignal, env?: Record<string, string>): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolve) => {
    ...
    const spawnEnv: Record<string, string> = { ...baseEnv };
...
`;
  ...
  pi.registerCommand("convex-auth", {
    ...
    async handler(_args, ctx) {
      ...
      let authType: "deploy_key" | "anonymous";
      let deployKey = "";
      ...
      if (isCI) {
        ...
        // Interactive mode
        const useAnonymous = await ctx.ui.confirm(
          "Use anonymous mode?",
          "Anonymous = no auth needed, useful for CI agents"
        );
  ...
  pi.registerCommand("convex-connect", {
    ...
    async handler(_args, ctx) {
      ...
      if (existingNames.length > 0) {
        const addNew = await ctx.ui.confirm(
          "Add new connection?",
      ...
      const type = await ctx.ui.confirm(
        "Is this local development?",
        "Local (localhost) or Cloud/Self-hosted?"
      );

      const url = await ctx.ui.input(
        "URL:",
        type ? "http://127.0.0.1:3210" : "https://your-project.convex.cloud"
      );
      ...
      let deployKey = "";
      let authType: "deploy_key" | "anonymous" | null = null;
      
      if (!type) {
        // Cloud/Self-hosted - ask for auth type
        const useAnonymous = await ctx.ui.confirm(
          "Use anonymous mode?",
          "Anonymous = no auth (CI agents). No = use deploy key."
        );
  ...
  pi.registerCommand("convex-disconnect", {
    ...
    async handler(_args, ctx) {
      ...
      const toRemove = await ctx.ui.input(
  ...
  pi.registerCommand("convex-project", {
    ...
    async handler(_args, ctx) {
      ...
      const useDetected = await ctx.ui.confirm(
        "Use detected project?",
        `Found: ${detected?.name || "none"}`
      );
      ...
      const projectPath = await ctx.ui.input(
        "Project path:",
      ...
      const projectName = await ctx.ui.input(
        "Project name:",
        project?.name || require("path").basename(projectPath)
      );
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      try {
        ...
        const functions = spec.functions?.map((f: any) => f.identifier) || [];
        const conn = getActiveConnection();

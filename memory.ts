...
export function updateProjectMemory(
  ...
  projectPath: string,
  connection: string,
  updates: Partial<Pick<ProjectMemory, "tables" | "functions" | "patterns">>
): void {
...
export function getProjectContext(projectName: string): string {
  const memory = loadMemory();
...
export function getActiveProjectMemory(): ProjectMemory | null {
  const memory = loadMemory();
  if (!memory.activeProject) return null;
  return memory.projects[memory.activeProject] || null;
}

export function addPattern(projectName: string, pattern: string): void {
  const memory = loadMemory();
  if (!memory.projects[projectName]) return;
